...
static void core_v_mcu_soc_realize(DeviceState *dev, Error **errp)
{
    ...
    qdev_connect_gpio_out(DEVICE(&s->timer), 1,
                          qdev_get_gpio_in(DEVICE(qemu_get_cpu(0)),
                                           IRQ_LOCAL_TIMER_HI));
    qdev_connect_gpio_out(DEVICE(&s->timer), 2,
                          qdev_get_gpio_in(DEVICE(qemu_get_cpu(0)),
                                           IRQ_LOCAL_TIMER_LO));
    ...
    for (i = 0; i < UDMA_N_SPI; i++) {
        ...
        DeviceState *flash = ssi_create_peripheral(s->udma_spi[i].bus,
                                                   "n25q256a13");
        qdev_connect_gpio_out_named(DEVICE(&s->udma_spi[i]), "spi_cs", 0,
    ...
    for (i = 0; i < UDMA_N_I2C; i++) {
        ...
        smbus_eeprom_init_one(s->udma_i2c[i].bus, 0x6F, eeprom_buf);
#endif
    }
