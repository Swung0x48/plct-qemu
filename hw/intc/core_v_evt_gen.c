...
static void round_events(void *opaque)
{
    ...
    if (!s->fc_ready) {
        bool ack = RISCV_CPU(qemu_get_cpu(0))->env.ack_valid;
        if (ack) {
            RISCV_CPU(qemu_get_cpu(0))->env.ack_valid = false;
    ...
    timer_mod(&s->r_timer, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + 10000);
}
...
static void core_v_evt_gen_write(void *opaque, hwaddr addr,
...
static uint64_t core_v_evt_gen_read(void *opaque, hwaddr addr,
                                    unsigned size)
{
    CoreVEvtGenState *s = opaque;
    uint32_t value = 0;
...
static void core_v_evt_gen_realize(DeviceState *dev, Error **errp)
{
    ...
    timer_init_ns(&s->r_timer, QEMU_CLOCK_VIRTUAL, round_events, s);
